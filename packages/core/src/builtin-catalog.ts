...
const BUILTIN_PLUGINS: readonly BuiltinPluginCatalogEntry[] = [
  ...
  plugin("telegram", "channel", "openclaw", "Telegram bot channel backend with webhook or long-poll delivery.", "high", "channel-telegram", "capability-packs/telegram", undefined, setup({ channels: ["telegram"], setupUrls: ["https://core.telegram.org/bots/tutorial", "https://core.telegram.org/bots/api#setwebhook"], notes: ["Telegram may be unavailable in some regions; the backend remains useful for deployments where Bot API access is reachable.", "The bundled channel pack provides setup, readiness, and update-debugging tools so agents can guide non-technical users without exposing bot tokens."] })),
  ...
  plugin("artifact-studio", "artifacts", "muster", "PDF, spreadsheet, presentation, image, report, and dashboard generation workflows.", "medium", "artifacts", "capability-packs/artifact-studio", undefined, setup({ notes: ["Local markdown, CSV, and dashboard-manifest builders are enabled. Document/slide/PDF app connectors remain separate setup paths."] })),
