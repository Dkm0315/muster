...
- [ ] Web-search providers (Brave/DDG/Exa/Firecrawl/Gemini/Grok/Kimi/MiniMax/Perplexity/SearXNG/Tavily) — muster: **PARTIAL 🟡** — Brave + DuckDuckGo are implemented in `packages/core/src/tool-registry.ts`; provider-pack expansion remains — effort L
...
- [ ] Hybrid vector+keyword memory search w/ provider (`memorySearch.provider`: OpenAI/Gemini/Voyage/Mistral/local/Bedrock) — muster: **PARTIAL 🟡** — SQLite/FTS keyword index ✅; add vector backend + provider config — effort L
